...
def send_overdue_book_returns():
    ...
    if overdue_borrows:
        for barrow in overdue_borrows:
            ...
            expected_return_date = datetime.strptime(str(barrow.expected_return), date_format)  # первая дата
            today = datetime.strptime(str(datetime.today().date()), date_format)  # вторая дата
        ...
        asyncio.run(
            send_message_to_channel(
                text=f"No borrowings overdue today!"
            )
        )
